...
.certs__section-label--progress {
  color: var(--text-muted);
  margin-top: 40px;
}
.certs__grid--completed {
  margin-bottom: 8px;
}
.certs__grid {
  display: grid;
...
if ($LASTEXITCODE -ne 0) {
  git commit -m "Add CCMA card, certificate lightbox viewer, 9 Certificates stat"
}
git push
Write-Host ""
Write-Host "Done! Site updated and pushed." -ForegroundColor Green
Write-Host ""
